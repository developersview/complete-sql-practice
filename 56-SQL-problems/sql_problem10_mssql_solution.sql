...
INSERT INTO amazon_transactions VALUES (1,109,'milk','2020-03-03 00:00:00',123),
...
-- solution 1
WITH cte_next_purchase AS(
	SELECT 
		user_id, 
		created_at,
		LAG(created_at) OVER(PARTITION BY user_id ORDER BY created_at DESC) AS next_purchase
	FROM 
		amazon_transactions
)
SELECT 
...
FROM 
	cte_next_purchase
WHERE 
	DATEDIFF(DAY, created_at, next_purchase) <= 7;
...
WHERE 
	DATEDIFF(DAY, a.created_at, b.created_at) <= 7;
