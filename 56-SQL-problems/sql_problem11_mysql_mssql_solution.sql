...
and order records by the product id in ascending order. 
...
(2, 'apple_fuji', 'produce', 'lb', 1.44, 0.43, 2),(3, 'orange', 'produce', 'lb', 1.02, 0.37, 2),
...
SELECT 
    i.product_name, 
    COUNT(t.transaction_id) AS transcation_count
FROM
    excel_sql_inventory_data i
        JOIN
    excel_sql_transaction_data t ON i.product_id = t.product_id
